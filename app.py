...
app.config['MYSQL_PASSWORD'] = 'sdg321'
app.config['MYSQL_DB'] = 'new_trendit'
...
def content_based_filtering(product_id):
    ...
    recommend_id = []
    cate_level = ['length', 'gold', 'clean_text', 'design', 'chain', 'silver', 'weight', 'color', 'formal', 'casual', 'luxury']
...
def index():
    ...
    earring = cur.fetchall()
    values = 'nailart'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    nailart = cur.fetchall()
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    shoes = cur.fetchall()
    # Close Connection
    cur.close()
    return render_template('home.html', earring=earring, nailart=nailart)
...
def earring():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('earring.html', earring=products, form=form)
    if 'view' in request.args:
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                mysql.connection.commit()
        return render_template('view_product.html', x=x, earrings=product)
        ...
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, earrings=product, form=form)
    return render_template('earring.html', earring=products, form=form)

@app.route('/nailart', methods=['GET', 'POST'])
def nailart():
    form = OrderForm(request.form)
    # Create cursor
    cur = mysql.connection.cursor()
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('nailart.html', nailart=products, form=form)
    if 'view' in request.args:
        product_id = request.args['view']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                cur.execute("INSERT INTO product_view(user_id, product_id) VALUES(%s, %s)", (uid, product_id))
                mysql.connection.commit()
        return render_template('view_product.html', x=x, nailarts=product)
    elif 'order' in request.args:
        product_id = request.args['order']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, nailarts=product, form=form)
    return render_template('nailart.html', nailart=products, form=form)
        ...
        if name and price and description and available and category and item and code and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    ...
                    if category == 'earring':
                        level = request.form.getlist('earring')
                        for lev in level:
                            yes = 'yes'
                        ...
                        level = request.form.getlist('nailart')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
...
if __name__ == '__main__':
    app.run(host='192.168.55.55', port='5000', debug=True)
